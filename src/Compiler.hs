...
compile :: Card -> [Card] -> FilePath -> FilePath -> [Deployment]
compile card allCards currentDir home = dps
  where ((_,_),dps) = runSparkCompiler (initialState card allCards currentDir home) compileDeployments
